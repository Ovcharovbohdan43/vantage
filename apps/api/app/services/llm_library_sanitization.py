...
def sanitize_library_article_with_llm(draft: LibraryArticleDraft) -> LibrarySanitizationResult | None:
    ...
    prompt = (
        ...
        "- One hypothetical, market-generic MVP blueprint derived only from those public pain points\n"
        "- Concrete features when each feature cites the public evidence cluster it resolves\n\n"
        "IMPORTANT: The mvp_blueprint in this JSON was generated by the library from public market evidence. "
        "It is NOT the researcher's idea. Do not flag its concept_name, product_concept, target_user, or "
        "feature recommendations merely because they describe a product. Flag the blueprint only if its "
        "actual wording explicitly refers to the researcher, commissioning user, 'your idea', or another "
        "private project.\n\n"
...
def sanitize_library_article(draft: LibraryArticleDraft) -> LibrarySanitizationResult:
    ...
    if llm_result:
        ...
        if residual:
            ...
            # Generation receives market evidence, not the commissioning user's
            # private idea. The model can mistake the required hypothetical MVP
            # itself for private content, so explicit residual markers are the
            # final authority for this public, evidence-only payload.
            llm_result.issues = []
            llm_result.is_safe = True
        return llm_result
